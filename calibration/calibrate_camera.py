...
image_path_pattern = '../data/calibration_images/*.jpg'
output_folder = '../data/calibration_coefs'

#
# TASK: Specify these
#
board_size = (4, 7) #PLACEHOLDER # Number of internal corners of the checkerboard (see tutorial)
square_size = 1.0 #PLACEHOLDER # Real world length of the sides of the squares
...
np.save(join(output_folder, 'K.npy'), K) # Intrinsic matrix (3x3)
np.save(join(output_folder, 'dc.npy'), dc) # Distortion coefficients
np.save(join(output_folder, 'mean_errors.npy'), mean_errors)
np.save(join(output_folder, 'std_int.npy'), std_int) # Standard deviations of intrinsics (entries in K and distortion coefficients)
